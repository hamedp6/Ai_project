...
class DotsAndBoxesAgent:
    ...
    def minimax(self, is_max, free_lines, r, c, o, main_cells, woncells):
        if not bool(free_lines):
            moves = [r, c, o]
            return [moves, woncells[0] - woncells[1]]
        cells = main_cells
        best_move = []
        free = free_lines
        if is_max:
            ...
            for i in free:
                ...
                cells[r][c][o] = 1
                flag = woncells
                if self.wonCell(r, c, o, cells):
                    flag[0] = flag[0] + 1
                    free.remove(i)
                    output = self.minimax(True, free, r, c, o, cells, flag)
                    score = output[1]
                    move = output[0]
                    if score > max_score:
                        max_score = score
                    ...
                    output = self.minimax(False, free, r, c, o, cells, flag)
                    score = output[1]
                    move = output[0]
                    if score > max_score:
                        max_score = score
    ...
    def next_action(self):
        ...
        free = free_lines
        won_cells = [0, 0]
        moves = self.minimax(False, free, 1, 1, 'h', self.cells, won_cells)
        return moves[0][0], moves[0][1], moves[0][2]
